            ...
            HELPER.ajax({
                ...
                complete: (response) => {
                    ...
                    myQ.enqueue(function (next) {
                        if (response.menu.success) {
                            ...
                            $.each(response.menu.data, (i, v) => {
                                ...
                                if (sidebarGroup != v.menu_group && v.menu_group) {
                                    sidebarList.push([`
                                        <li class="heading">
                                            <h3 class="uppercase">${v.menu_group}</h3>
                                        </li>
                                    `]);
                                }
                                // set menu list
                                sidebarList.push([`
                                    <li class="list-menu list-menu-${v.menu_id} tooltips"
                                        data-container="body"
                                        data-placement="right"
                                        data-html="true"
                                        data-original-title="${v.menu_title}"
                                    >
                                        <a href="javascript:void(0);" onclick="HELPER.loadPage(this)"
                                            data-id="${v.menu_id}"
                                        >
                            ...
                            // append to sidebar
                            $(`.page-sidebar-menu`).html(sidebarList.join(''));
                        } else {
                            HELPER.showMessage({
                        ...
                        // set modul active
                        $(`.modul-menu`).removeClass('active');
                        $(`.modul-menu-${page.id}`).addClass('active');
                        next();
                ...
                complete: (response) => {
                    ...
                    if (! response.signined){
                        ...
                        myQ.enqueue(function (next) {
                            ...
                            if (response.menusub) {
                                setMenuSub.push([`
                                    <div class="btn-group btn-group-md btn-group-solid margin-bottom-20 owl-carousel" id="btn-group">
                                `]);
                                $.each(response.menusub, (i,v) => {
                                    const isActiveMenu = (v.id === response.id) ? 'active-menu' : '';
                                    setMenuSub.push([`
                                        <button
                                            type="button"
                                            class="btn grey ${isActiveMenu}"
                                            onclick="HELPER.loadPage(this)"
                                            data-id="${v.id}"
                                            id="btn-menu"
                                        >
                            ...
                            $.each(response.breadcrumb, (i,v) => {
                                setBreadcrumb.push([`
                                    <li>
                                        ...
                                        ${((response.breadcrumb.length-1) != i)? `<i class="fa fa-angle-right"></i>`:``}
                                    </li>
                                `]);
                            });
                            ...
                            // set active menu
                            $(`li.list-menu`).removeClass('active');
                            if (response.level > 1) {
                                // set active for parent menu
                                $(`li.list-menu-${response.parentId}`).addClass('active');
                            } else {
                                // set active for current menu
                                $(`li.list-menu-${response.id}`).addClass('active');
                            }
                            // set sub menu list
