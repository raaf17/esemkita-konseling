/*$(function(){
    $('[data-menuid="0001"]').trigger('click');
})*/
...
var HELPER = function()
{
    var loadBlock = function(message)
    {
        $.blockUI({ 
            ...
            css: {border: 'none', backgroundColor: 'rgba(47, 53, 59, 0)', 'z-index': '99999'}
        }); 
    }
    ...
    return {
                ...
                '<div class="modal-dialog">'+
                    '<div class="modal-content">'+
                        '<div class="modal-header">'+
                            ...
                            '<h4 class="modal-title" id="title_berita_detail"><i class="icon-settings"></i> Pengaturan Akun</h4>'+
                        '</div>'+
        ...
        onUpdatePassword: function(){
            ...
            /*HELPER.save({
                form : 'set_account_form',
                url : APP_URL+'Siswa/upDatePassword',
                confirm: true,
                callback : function(success,id,record,message, data){
                    if (success) {
                        $('.rb').attr('style','');
                        $('#modal_setting').modal('hide');
                    }else{
                        $('.rb').attr('style','');
                        var rRes = $.parseJSON(data.responseText);
                        $.each(rRes, function(i,v){
                            $('[name="'+v+'"]').attr('style', 'border: 1px solid red;')
                        });
                    }
                }
        ...
        logout: function()
        {
            HELPER.confirm({
                ...
                callback: function(result){
                    if (result) {
                        localStorage.setItem('is_session', 'false');
                        $.ajax({
        ...
        loadPage: function(el, isPopState=false, lastUrl='')
        {
            loadBlock();
            var is_notif = false;
            var data_notif = {};
            ...
            // DETAIL FAQ
            var detail_faq = $(el).data();
            if (typeof detail_faq.isdetailfaq != 'undefined' && detail_faq.isdetailfaq == true) {
                localStorage.setItem("faq_detail_id", detail_faq.kategori+'-'+detail_faq.id);
            }else{
                localStorage.removeItem("faq_detail_id");
            }

            $('.parent_menu').removeClass('active');
            $('.li_sub').removeClass('active');
            $(el).parent('li').addClass('active');
            $($(el).parents('li')[1]).addClass('active');
            var page     = $(el).data();
            var dataSend = {
                'con':page.con,
                'menuid':page.menuid, 
            ...
            menuid       = page.menuid;
            $($($('.li_sub_'+page.menuid).parent()).parent()).addClass('active');
            $('.li_sub_'+page.menuid).addClass('active');
            $('.parent_menu_'+page.menuid).addClass('active');
            $.ajax({
                url: APP_URL + "Main/getPage",
                data: (isPopState) ? el : dataSend,
                type: "POST",
                complete: function(pages)
                {
                    ...
                    if (!isPopState) {
                        var mx = resp_object.menu_kode.split('-');
                        var mx2 = (typeof mx !='undefined') ? mx[0].toLowerCase() : '';
                        window.history.pushState({'menu_id':resp_object.menu_id}, resp_object.menu_title, mx2);
                    }

                    var key_st = localStorage.getItem("is_session");
                    if (key_st == 'true') {
                    ...
                    $.when(function(){
                        $("#idcontent").html(atob(resp_object.view)).trigger('click');
                        $('#icon-title').html('<i class="flaticon-list-2"></i>');
                        $('#icon-form').html('<i class="flaticon-edit-1"></i>');
                        if (resp_object.menu_title !='Beranda') {
                            $('#con_breadcrumb').html(atob(resp_object.breadcrumb));
                            $('#pagetitle').html('.:: '+resp_object.menu_title).css({'font-weight':'700','color':'#3e5c74'});
                            // $('#pagetitle').html('.:: '+resp_object.menu_title+" <small>"+title+"</small>").css({'font-weight':'700','color':'#3e5c74'});
                            $('.header').css('margin-bottom', '23px');
                            $('#slide_dashboard').css('display','none');
                        } else {
                            $('#con_breadcrumb').html('');
                            $('#pagetitle').html('');
                            $('.header').css('margin-bottom', '0px');
                            $('#slide_dashboard').css('display','block');
                        }

                        var html_faq = new Array();
                        html_faq.push([
                            '<div class="col-md-12 margin-bottom-10">'+
                                '<h2 class="title-header-blue">FAQ</h2>'+
                                '<div class="portlet">'+
                        ...
                        $.each(resp_object.faq, function(i,v){
                            var hr_ = '';
                            if ((i+1) < resp_object.faq.length) {
                                hr_ = '<hr style="margin: 10px 0;"/>';
                            }
                            html_faq.push([
                                '<a href="javascript:;" data-id="'+v.faq_id+'" data-kategori="'+v.faq_kategori+'" data-isdetailfaq="true" class="timeline-body-title font-blue-hoki" onclick="HELPER.loadPage(this)" data-menuid="0005" data-con="Faq-View" data-title="FAQ&#39;s" style="font-size: 13px;">'+
                                    '<li>'+
                                        v.faq_title+
                                    '</li>'+
                                '</a>'+
                        ...
                        html_faq.push(['</div></div></div></div></ol>']);
                        html_faq.push([
                                        '</div>'+
                        ...
                        if (resp_object.menu_title !='Beranda' && resp_object.menu_title !='Berita' && resp_object.menu_title !='Kegiatan') {
                            $('.berita_kegiatan').css('display','block');
                            if (resp_object.menu_id !='0005') {
                            ...
                            $('.faq').css('display','block');
                            $('.berita_kegiatan').css('display','none');
                        }
                        
                        if (resp_object.menu_id =='00044') {
                       ...
                       /* var container = $("#idcontent");
                        ...
                        // console.log(resp_object.notif_update.success);
                       /* if (typeof resp_object.notif_update != 'undefined') {
                            if (resp_object.notif_update.success) {
                                HELPER.ajax({
                                    url: APP_URL+'Main/getNotification',
                                    success: function(dSuc2) {
                                        var html_notif = new Array();
                                        html_notif.push([
                                            '<a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true" style="text-decoration: none;">'+
                                                '<i class="icon-bell" style="top: 4px;"></i>'+
                                                '<span class="badge badge-default" style="background-color: #f9ac00;"> '+dSuc2.notif_total+'</span>'+
                                            '</a>'+
                                            '<ul class="dropdown-menu" style="width: 280px; border-radius: 2px!important;">'+
                                                '<li class="external" style="padding: 10px; background-color: #f9ac00; border-top-right-radius: 3px!important; border-top-left-radius: 3px!important;">'+
                                                    '<h4 class="bold" style="color: #fff; margin-bottom: 0px;">'+dSuc2.notif_total+' Pemberitahuan Baru</h4>'+
                                                '</li>'+
                                                '<li style="padding: 0px;">'+
                                                    '<ul class="dropdown-menu-list scroller" style="height: 250px; padding: 0px;" data-always-visible="1" data-rail-visible1="0" data-handle-color="#637283">'+
                                        ...
                                        $('#header_notification_bar').html(html_notif.join(''));
                                    }
                                })
                            }
                        }*/
                        ...
                        if (page.isnotif == 1) {
                            ...
                            $('#tab_reversed_1_1').removeClass('active');
                            $('#tab_reversed_1_2').addClass('active');
                        }
                    }())
                },
        ...
        aksi : function(role_update='', role_delete=''){
            return  '<a href="javascript:;" data-roleable="true" data-role="'+role_update+'" onclick="onEdit(this)" class="hide xremove_button btn btn-xs btn-success"> <i class="fa fa-edit"></i> Ubah </a>&nbsp;&nbsp;'+
            '<a href="javascript:;" data-roleable="true" data-role="'+role_delete+'" onclick="onDelete(this)" class="hide xremove_button btn btn-xs btn-danger"> <i class="fa fa-trash-o"></i> Hapus </a>';
        },
        ...
        tinymce: function($class){
            tinymce.init({
                ...
                external_plugins: {
                    ...
                    "filemanager": "http://192.168.11.5/spi/ng/plugin.min.js"
                },
            });
        },
...
var MAPS = function(window, document, undefined) {
    return {
        init: function(config) {
            config.lat = (typeof config.lat !='undefined') ? config.lat : -4.5130816;
            config.lng = (typeof config.lng !='undefined') ? config.lng : 106.3611956;
            var _config = $.extend({}, {
            ...
            var url = 'http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png'; //online
            var osmAttrib = '';
...
function twoDigitPad(num) {
    ...
    function onLogin()
    {
        HELPER.ajax({
            ...
            complete: function(data){
                if (data.success) {
                    localStorage.setItem("is_session", 'true');
                    window.location.reload();
                } else {
                ...
                $('.placeholder-no-fix').on('keypress', function(){
