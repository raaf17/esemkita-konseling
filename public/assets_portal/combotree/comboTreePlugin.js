...
;(function ( $, window, document, undefined ) {
    ...
    ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
        ...
        if (isThereSubs)
          itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '<i class="fa fa-caret-right icon-sm" style="padding-right: 3px; color: #767676;"></i>' : '<i class="fa fa-caret-down icon-sm" style="padding-right: 3px; color: #767676;"></i>') + '</span>';

        if (this.options.isMultiple)
            itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" class="icheck" />' : '') + sourceItem.title + '</span>';
        else
    ...
    ComboTree.prototype.toggleSelectionTree = function (item, direction) {
        ...
        if (direction === undefined){
            if ($(subMenu).is(':visible'))
                $(item).children('span.comboTreeParentPlus').html('<i class="fa fa-caret-right icon-sm" style="padding-right: 3px; color: #767676;"></i>'); //$(item).children('span.comboTreeParentPlus').html("+");
            else
                $(item).children('span.comboTreeParentPlus').html('<i class="fa fa-caret-down icon-sm" style="padding-right: 3px; color: #767676;"></i>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
				...
				if ($(ctItem).parent('li').hasClass('ComboTreeItemParent')) {
					...
					subMenu.each(function() {
						...
						if ($(ctItem).children('input[type="checkbox"]').first().prop("checked")!==$input.prop('checked')) {
                            $($($($($('#comboTree644421ComboTreeSourceUl1').children()).children()).children()).children()).addClass('checked')
    ...
    ComboTree.prototype.setMultipple = function(el) {
        this.options.isMultiple = el;
        this.constructorFunc(this.elemInput, this.options);
    };
