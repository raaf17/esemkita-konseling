...
namespace App\Modules\Konseling\Models;
...
class KonselingModel extends Model
{
    ...
    protected $allowedFields    = ['id', 'id_kelas', 'id_guru', 'id_layanan', 'id_siswa', 'tanggal', 'jam', 'deskripsi', 'status', 'created_at', 'updated_at'];
    protected $useTimestamps    = true;
    protected $useSoftDeletes   = false;
    protected $beforeInsert     = ['generateUuid'];

    protected $column_order = array(null, 'nama_kelas', 'nama_siswa', 'nama_layanan', 'tanggal', 'jam', 'deskripsi', null);
    ...
    private function _get_datatables_query($status = null)
    {
        $i = 0;
        if ($status == 'pending') {
            $this->dt->where('konseling.status', 0);
        } elseif ($status == 'approved') {
            $this->dt->where('konseling.status', 1);
        } elseif ($status == 'unapproved') {
            $this->dt->where('konseling.status', 2);
        } elseif ($status == 'done') {
            $this->dt->whereIn('konseling.status', [1, 2]);
        }
    ...
    public function DashboardKonseling()
    {
        $db = \Config\Database::connect();
        $query = $db->table('konseling')
            ->select('layanan.nama_layanan, COUNT(konseling.id) AS jumlah')
            ->join('kelas', 'kelas.id = konseling.id_kelas')
            ->join('guru', 'guru.id = konseling.id_guru')
            ->join('layanan', 'layanan.id = konseling.id_layanan')
            ->join('siswa', 'siswa.id = konseling.id_siswa')
    ...
    public function allKonseling()
    {
        ...
        $query = $db->table('konseling')
            ...
            ->join('siswa', 'siswa.id = konseling.id_siswa', 'left')
            ->limit(4)
            ->orderBy('konseling.created_at', 'DESC');
