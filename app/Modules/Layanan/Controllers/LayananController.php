...
class LayananController extends BaseController
{
    ...
    public function index()
    {
        ...
        return view('App\Modules\Layanan\Views\index', $data);
    }

    public function listData()
    {
        $request = \Config\Services::request();
        $datamodel = new LayananModel();
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($lists as $list) {
                ...
                $buttonEdit = '<div class="d-flex"><a href="#" class="btn btn-sm btn-light-warning btn-action edit-layanan-btn p-0 p-2 me-2" data-id="' . $list->id . '"><i class="ki-outline ki-notepad-edit fs-4"></i></a>';
                $buttonDelete = '<a href="#" class="btn btn-sm btn-light-danger delete-layanan-btn p-0 p-2" data-id="' . $list->id . '"><i class="ki-outline ki-basket fs-4"></i></a></div>';
    ...
    public function store()
    {
        ...
        if ($request->isAJAX()) {
            ...
            $this->validate([
                'nama_layanan' => [
                    ...
                    'errors' => [
                        ...
                        'is_unique' => 'Nama layanan ini sudah ada',
                    ]
                ]
            ]);
            ...
            if ($validation->run() === FALSE) {
                ...
                $query = $this->layanan->insert($data);
    ...
    public function update()
    {
        ...
        if ($request->isAJAX()) {
            ...
            if ($validation->run() === FALSE) {
                ...
                $query = $this->layanan->where('id', $id)->set($data)->update();
    ...
    public function export()
    {
        ...
        $activeWorksheet->setCellValue('A1', 'DATA LAYANAN');
        ...
        $activeWorksheet->setCellValue('B6', 'NAMA LAYANAN');
        ...
        foreach ($layanan as $key => $value) {
            ...
            $activeWorksheet->setCellValue('B' . $column, $value->nama_layanan);
        ...
        $filename = 'data-layanan_' . date('d-m-y-H-i-s') . '.xlsx';
    ...
    public function import()
    {
        ...
        if ($request->isAJAX()) {
            ...
            if ($extension == 'xlsx' || $extension == 'xls') {
                ...
                $layanan = $spreadsheet->getActiveSheet()->toArray();
                ...
                foreach ($layanan as $key => $value) {
                    ...
                    if (!empty($value[1])) {
                        ...
                        $exists = $this->layanan->where('nama_layanan', $nama_layanan)->countAllResults();
    ...
    public function multipleDelete()
    {
        ...
        if ($request->isAJAX()) {
            ...
            if (!empty($id)) {
                ...
                foreach ($id as $item) {
                    $query = $this->layanan->delete($item);
                    if (!$query) {
                        $querySuccess = false;
                        break;
                    }
                ...
                if ($querySuccess) {
                    return $this->response->setJSON([
                        'status' => 1,
                        'token' => csrf_hash(),
                        'msg' => "$jumlahData data layanan berhasil dihapus"
                    ]);
                } else {
                    return $this->response->setJSON([
                        'status' => 0,
